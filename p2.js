...
if (bloggedin) {
	...
	//window.onload = loadURL(baseurl+'/index.cfm?fuseaction=user');
	window.onload = function() {loadURL(baseurl+'/index.cfm?fuseaction=user');};
} else {
...
function changeSection(section_id,section_str) {
	var mytds = document.getElementsByTagName('td'); 
	var c=0; 
	for(var i=0; i<mytds.length; i++) { 
		if (mytds[i].innerHTML.match(section_str)) {  
			c++; 
			if (c==4) {
				//alert(mytds[i].innerHTML);
				void(mytds[i].innerHTML=unescape(document.getElementById(section_id).innerHTML));
			}
		} 
	}
}
if (document.getElementById('ss_aboutme')) {
	changeSection('ss_aboutme','About me:');
}
if (document.getElementById('ss_whoidlike')) {
	//var t=setTimeout("changeSection('ss_whoidlike','Who I\'d like to meet:');",100);
	changeSection("ss_whoidlike","Who I'd like to meet:");
}
